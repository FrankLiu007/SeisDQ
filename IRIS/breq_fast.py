        ...
        for sId, station in pool.stations.items():
            ...
            if pool.pars["data_apply_mode"]=="per_station":                
                if sId in requests:
                    ...
                    pool.pars["breq_head"]['.LABEL']=stnet+"."+stnm+"-"+str(sId)
                    head=generate_breq_head(pool.pars["breq_head"])
                    requests[sId]=head+"\n"+tmp
            elif pool.pars["data_apply_mode"]=="per_event":
                if eId in requests:
                    requests[eId]=requests[eId]+"\n"+tmp
                else:
                    pool.pars["breq_head"]['.LABEL']="eventId-"+str(eId)
                    head=generate_breq_head(pool.pars["breq_head"])
                    requests[eId]=head+"\n"+tmp
...
def SendRequest(breqs, head, data_apply_mode):
