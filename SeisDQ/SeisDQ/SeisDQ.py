...
from utils import event, station, common
from utils.TravelTime import calculate_travel_time, get_distance


class DataPool:
    def __init__(self, args_par_path):
        self.pars = common.read_pars(args_par_path)
        self.events = event.read_events(self.pars['events'])
        self.stations = station.read(self.pars['station_path'])
        self.all_data = {}
